...
void drawVisionCheckingScreen(uint8_t dotCount, bool fullRedraw) {
  ...
  if (fullRedraw) {
    ...
    tft.fillRect(0, 0, 160, 3, 0x07FF); // Cyan accent bar
...
void drawVisionValidScreen() {
  tft.fillScreen(tft.color565(10, 60, 25)); // Dark green
  tft.fillRect(0, 0, 160, 3, 0x07E0); // Bright green accent bar

  // Tick mark: left arm (54,58)-(68,74), right arm (68,74)-(106,38), 7-pixel stroke
  for (int t = 0; t <= 6; t++) {
...
void drawVisionInvalidScreen(const char* locationLabel) {
  tft.fillScreen(tft.color565(70, 10, 10)); // Dark red
  tft.fillRect(0, 0, 160, 3, 0xF800); // Red accent bar

  // Cross 'X' icon centered at approximately (80, 30)
  for (int t = -2; t <= 2; t++) {
